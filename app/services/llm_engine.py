...
class LLMEngine:
    ...
    async def extract_toc_json(self, text_pages: str, extra_instruction: str = "") -> list:
        ...
        prompt = (
            ...
            "4. title — это КОРОТКИЙ заголовок раздела, как написано в оглавлении: "
            "номер плюс название, до 100 символов. НЕ продолжай за пределы строки "
            "заголовка, не добавляй описания или первое предложение раздела. "
            "Последнее число строки оглавления — это номер страницы, оно идёт в "
            "отдельное поле \"page\", в title его не включай.\n"
            "5. СОХРАНЯЙ иерархический номер раздела в title дословно: «1.», «1.2», "
            "«1.2.3», «§ 1.4», «Глава 5». Пример строки «1.2.1 Абстракция .... 6» → "
            "{\"title\": \"1.2.1 Абстракция\", \"page\": 6}.\n"
            "6. Используй язык оригинала. ЗАПРЕЩЕНО переводить заголовки на "
            "другие языки (особенно китайский, японский, корейский, арабский).\n"
            "7. Извлекай ВСЕ пункты оглавления, включая подразделы (1.1, 1.2, ...). "
            "Не выдумывай заголовки, которых нет в тексте.\n"
            "8. Если оглавления нет — верни {\"items\": []}.\n"
            f"{correction}\n"
            f"Текст:\n---\n{text_pages}\n---"
        )
        try:
