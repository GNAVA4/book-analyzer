...
class LLMEngine:
    ...
    async def extract_toc_json(self, text_pages: str, extra_instruction: str = "") -> list:
        ...
        prompt = (
            ...
            "4. title должен быть КОРОТКИМ заголовком (до 100 символов). НОМЕР "
            "СТРАНИЦЫ (последнее число строки) идёт в отдельное поле \"page\".\n"
            "5. СОХРАНЯЙ иерархические номера разделов («1», «1.2», «1.2.3», "
            "«§ 1.4», «Глава 5», «5.7.10») КАК ЧАСТЬ title. Они есть в исходном "
            "тексте перед названием — оставляй их в title дословно. НЕ обрезай. "
            "Пример: исходная строка «1.2.1 Абстракция .... 6» → "
            "{\"title\": \"1.2.1 Абстракция\", \"page\": 6}, НЕ «Абстракция».\n"
