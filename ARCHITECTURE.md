# Архитектура Book Analyzer

_Текущая полная архитектура системы парсинга книг._

_Все диаграммы — Mermaid. Рендерятся в GitHub, VSCode (с расширением Mermaid Preview), на [mermaid.live](https://mermaid.live), а также в сопутствующем HTML-файле._
...
flowchart TD
    ...
    VALID_TOC -->|Нет| FULL_TEXT
    VALIDATE_OCR --> FULL_TEXT["clean_footer_header<br/>от колонтитулов<br/>pdf_utils.py"]

    FULL_TEXT --> MAP_STAGE[/"map_sequence<br/>см. диаграмму Mapping ниже"/]
    ...
    SLICE --> CONF_CHECK{confidence<br/>≥ 0.85?}
...
| **Книга без префиксов** (digital-design, **Дефект А**) | `parse_pdf_neural` → `build_toc` (ocr_llm без N.N.N префиксов) → `map_sequence` (regex находит первое generic вхождение → каша) → 79 секций <100 chars |
